...
class LinearRegressionClass(Classifier):
    ...
    def learn(self, Xtrain, ytrain,lamda):
        """ Learns using the traindata """
        ...
        self.weights = np.dot(np.dot(np.linalg.inv(np.add(np.dot(Xtrain.T,Xtrain)/numsamples,lamda*np.identity(Xtrain.shape[1]))), Xtrain.T),yt)/numsamples
...
#class KernelbasedLC(Classifier):
#    
#    def __init__(self,number):
#        self.weights = None
#        self.center=None
#        self.exp=[x for x in range(1,1000)]
#        self.Ktrain=None
#    
#    def learn(self, Xtrain,ytrain):
#        alpha = 150
#        self.center=Xtrain[self.exp,:]
#        self.Ktrain=np.dot(Xtrain,self.center.T)
#        length = len(self.exp)
#        IdentityMAt = np.identity(length)
#        self.weights = np.dot(np.dot(np.linalg.inv(self.Ktrain+np.dot(alpha,IdentityMAt)), Xless.T),ytrain)
...
class LogitRegL2(Classifier):
    ...
    def learn(self, Xtrain, ytrain,lamda):
        ...
        while self.getTolerance(NewWeights, oldweights)>tolerance:
            ...
            Gradient=np.dot(Xtrain.T, np.subtract(ytrain, p))-2*lamda*NewWeights
            oldweights=NewWeights+np.dot(Hessian,Gradient)
        self.weights=oldweights
